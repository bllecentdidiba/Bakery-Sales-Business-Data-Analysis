--Business Problem: Identify which products are "Stars" (high revenue, high volume) vs. "Dogs" (low revenue, low volume). 
WITH product_performance AS (
    SELECT p.product_name, p.gluten_free,p.is_gluten_free,
      ...
      ROUND(AVG(p.cost)::numeric, 2) as avg_cost,
        ROUND(AVG(p.profit_margin)::numeric, 2) as avg_profit_margin
    FROM dim_product p
